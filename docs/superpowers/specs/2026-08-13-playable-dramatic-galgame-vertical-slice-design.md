...
The internal causal slice is shorter:

```text
pre-generated opening
  -> choice 1: publicly trust or question Alice
  -> choice 2: new evidence challenges the stance
  -> choice 3: carry the cost or abandon the commitment
  -> dynamic short ending
```

This 10-15 minute slice proves the engine before content expands to the final bounds.
...
- A third party may force all three characters to decide whether they can cooperate.
...
The pack may declare reusable conflict seeds, not beats:
...
conflict_seeds:
  - id: trust_vs_evidence
    ...
    activation: protagonist has publicly favored one account
    escalation_question: >
      When new evidence harms the favored person, will the protagonist
      revise the judgment or defend the relationship?
```

A seed establishes a productive conflict space. The runtime model decides how it manifests in the current state.
...
completion_requirements:
  - id: truth_understood
    all:
      - fact_revealed: notebook_holder
      - clue_chain_completed: notebook_disappearance

  - id: meaningful_bond
    any:
      - relationship_turning_point: alice_mutual_trust
      - relationship_turning_point: bob_earned_respect
      - relationship_turning_point: mina_shared_responsibility

  - id: accepted_cost
    any:
      - obligation_fulfilled
      - sacrifice_made
      - irreversible_stance_defended
```

The compiler contract is a recursive evidence expression with `all`, `any`, and typed leaf predicates. The initial supported leaf predicates are `fact_revealed`, `clue_chain_completed`, `relationship_turning_point`, `obligation_fulfilled`, `sacrifice_made`, and `irreversible_stance_defended`. Unknown predicates, empty groups, and requirements without evidence fail pack compilation. Understanding, relationship, and accepted cost must all have committed evidence.
...
- pack version and prompt version;
...
After selection, the runtime verifies the binding, re-simulates the proposal against current authority, commits the choice and performance atomically, and starts rolling generation for the new frontier.
...
- primary model direct-contract acceptance: at least 95%.
...
Run multiple seeds for at least these policies:
...
- all three choices carry distinct value conflicts;
- the next performance visibly acknowledges every choice;
- the first choice changes the situation by the third choice;
...
- every automated run terminates;
- the configured normal model has at least 95% direct contract acceptance;
...
2. Introduce dramatic state and semantic event contracts behind the existing store/reducer boundary.
3. Compile machine-verifiable completion requirements and reject empty evidence rules.
4. Implement dramatic assignments, choice contracts, promise scheduling, and no-op validation.
5. Replace the production turn orchestrator with the unified drama turn service.
6. Implement state-bound opening and next-choice candidate generation.
7. Update the Web client for immediate playback, semantic ending trace, and basic fixed assets.
8. Build the three-choice causal slice and pass deterministic, automated, and human gates.
9. Expand the same pack and engine to 6-10 choices without adding cafe-specific engine branches.
10. Remove obsolete runtime paths and correct all CLI, API, tests, and documentation.
